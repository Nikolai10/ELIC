...
This is a reimplementation of ELIC published in:
D. He and Z. Yang and W. Peng and R. Ma and H. Qin and Y. Wang:
...
import numpy as np
from elic_transforms import ElicAnalysis, ElicSynthesis
from checkerboard_helper import mux, demux_nonanchor, demux_anchor, demux_nonanchor_inverse, demux_anchor_inverse
...
class HyperSynthesisTransform(tf.keras.Sequential):
    ...
    def __init__(self):
        ...
        layers = [
            conv(320, (5, 5), name="layer_0", strides_up=2),
            conv(480, (5, 5), name="layer_1", strides_up=2),
...
class HYPMQW2022Model(tf.keras.Model):
    """Main model class."""
    ...
    def __init__(self, patchsize, lmbda,
        ...
        self.slice_depths = [16, 16, 32, 64, 192]
        offset = tf.math.log(scale_min)
        factor = (tf.math.log(scale_max) - tf.math.log(scale_min)) / (
                num_scales - 1.)
        self.scale_fn = lambda i: tf.math.exp(offset + factor * i)
        self.analysis_transform = ElicAnalysis(channels=[192, 192, 192, 320])
        self.synthesis_transform = ElicSynthesis(channels=[192, 192, 192, 3])
        ...
        self.cc_transforms = [
            SliceTransform(self.slice_depths[i] * 2) if i else None for i in range(num_slices)]
        self.context_predictions = [
            CheckerboardContext(self.slice_depths[i] * 2, kernel_size=(5, 5), strides=1, padding='same') for i in
            range(num_slices)]
        self.entropy_params = [EntropyParameters(self.slice_depths[i]) for i in range(num_slices)]
        ...
        self.decompress = tf.function(
            input_signature=3 * [tf.TensorSpec(shape=(2,), dtype=tf.int32)] +
                            (num_slices * 2 + 1) * [tf.TensorSpec(shape=(1,), dtype=tf.string)]
        )(self.decompress)
    ...
    def call(self, x, training):
        ...
        # En/Decode each slice conditioned on hyperprior, local context and previous slices.
        y_slices = tf.split(y, self.slice_depths, axis=-1)
        y_hat_slices = []
        y_bpps = []
        for slice_index, y_slice in enumerate(y_slices):
            ...
            local_ctx_anchor = tf.zeros(
                [tf.shape(y_slice)[0], tf.shape(y_slice)[1], tf.shape(y_slice)[2], tf.shape(y_slice)[3] * 2])

            if slice_index == 0:
                support = tf.concat([local_ctx_anchor, hyper_params], axis=-1)
            else:
                channel_ctx = self.cc_transforms[slice_index](tf.concat(support_slices, axis=-1))
                support = tf.concat([channel_ctx, local_ctx_anchor, hyper_params], axis=-1)

            gaussian_params = self.entropy_params[slice_index](support)
            ...
            # Step 1.2: properly quantize y_hat_slice (only anchor at this point)
            y_hat_slice_anchor = em_y.quantize(y_slice, loc=mu_anchor)

            # 2. Encode nonanchor (mimic)
            # ================================================================================

            # Step 2.1: calculate entropy params (context feature set to y_hat_slice_anchor)
            local_ctx = self.context_predictions[slice_index](y_hat_slice_anchor)
            zeros = tf.zeros_like(local_ctx)
            # mask anchor
            local_ctx_nonanchor = mux(local_ctx, zeros)

            if slice_index == 0:
                support = tf.concat([local_ctx_nonanchor, hyper_params], axis=-1)
            else:
                support = tf.concat([channel_ctx, local_ctx_nonanchor, hyper_params], axis=-1)
    ...
    def compress(self, x):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            y_slice_anchor_demux = demux_anchor(y_slice)
            mu, sigma = tf.split(gaussian_params, 2, axis=-1)
            mu_anchor_demux = demux_anchor(mu)
            sigma_anchor_demux = demux_anchor(sigma)

            # Step 1.3: Compress anchors
            y_slice_string_anchor = self.em_y.compress(y_slice_anchor_demux, sigma_anchor_demux, loc=mu_anchor_demux)
            y_strings_anchor.append(y_slice_string_anchor)

            # Step 1.4: Decompress anchors
            y_hat_slice_anchor_demux = self.em_y.decompress(y_slice_string_anchor, sigma_anchor_demux,
                                                            loc=mu_anchor_demux)

            # Step 1.5: DEMUX-inverse (restore anchor format)
            y_hat_slice_anchor = demux_anchor_inverse(y_hat_slice_anchor_demux)
            ...
            y_slice_nonanchor_demux = demux_nonanchor(y_slice)
            mu, sigma = tf.split(gaussian_params, 2, axis=-1)
            mu_nonanchor_demux = demux_nonanchor(mu)
            sigma_nonanchor_demux = demux_nonanchor(sigma)

            # Step 2.3: Compress nonanchors
            y_slice_string_nonanchor = self.em_y.compress(y_slice_nonanchor_demux, sigma_nonanchor_demux,
                                                          loc=mu_nonanchor_demux)
            y_strings_nonanchor.append(y_slice_string_nonanchor)

            # Step 2.4: Decompress nonanchors
            y_hat_slice_nonanchor_demux = self.em_y.decompress(y_slice_string_nonanchor, sigma_nonanchor_demux,
                                                               loc=mu_nonanchor_demux)

            # Step 2.5: DEMUX-inverse (restore nonanchor format)
            y_hat_slice_nonanchor = demux_nonanchor_inverse(y_hat_slice_nonanchor_demux)

            # 3. Merge y_hat_slice_anchor and y_hat_slice_nonanchor togehter (MUX)
            # ================================================================================

            y_hat_slice = y_hat_slice_anchor + y_hat_slice_nonanchor
            y_hat_slices.append(y_hat_slice)

        return (x_shape, y_shape, z_shape, z_string) + tuple(y_strings_anchor) + tuple(y_strings_nonanchor)

    def decompress(self, x_shape, y_shape, z_shape, z_string, *y_strings):
        """Decompresses an image."""
        assert len(y_strings) == self.num_slices * 2

        midpoint = len(y_strings) // 2

        y_strings_anchor = y_strings[:midpoint]
        y_strings_nonanchor = y_strings[midpoint:]
        ...
        y_hat_slices = []
        for slice_index, (y_string_anchor, y_string_nonanchor) in enumerate(zip(y_strings_anchor, y_strings_nonanchor)):
            # Model may condition on only a subset of previous slices.
            support_slices = (y_hat_slices if self.max_support_slices < 0 else
                              y_hat_slices[:self.max_support_slices])
            ...
            # Step 1.1: calculate entropy params (context feature set to 0)
            local_ctx_anchor = tf.zeros([y_shape[0], y_shape[1], self.slice_depths[slice_index] * 2])
            local_ctx_anchor = tf.expand_dims(local_ctx_anchor, 0)
            ...
            y_hat_slice_anchor_demux = self.em_y.decompress(y_string_anchor, sigma_anchor_demux, loc=mu_anchor_demux)
            ...
            y_hat_slice_nonanchor_demux = self.em_y.decompress(y_string_nonanchor, sigma_nonanchor_demux,
                                                               loc=mu_nonanchor_demux)
...
def get_dataset(name, split, args):
    ...
    with tf.device("/cpu:0"):
        if name == 'imagenet2012':
            data_dir = "/tf/notebooks/tensorflow_datasets/"  # adjust to your needs
            dataset = tfds.load(name, split=split, shuffle_files=True, data_dir=data_dir)
        else:
...
def train(args):
    ...
    if args.train_glob:
        ...
        # train_dataset = get_dataset("imagenet2012", "train", args)
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--model_path", default="res/hypmqw2022",
    ...
    train_cmd.add_argument(
        "--train_path", default="res/train_hypmqw2022",
